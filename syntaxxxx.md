...
if a user notices all their drives/printers/scanners/ETC (firmware is stored somewhere) are active (making noises, blinking lights, ETC), and uses another system (phone, ETC) to query the forum/IRC/ETC; "the compiler is just using those resources to optimise the compilation process; only the computational aspects, so DON'T PANIC. :)"
